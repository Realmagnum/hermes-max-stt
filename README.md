...
Для быстрой проверки работоспособности MAX выполните скрипт диагностики (должен быть в `scripts/diagnose.sh` репозитория плагина):
...
Скрипт проверяет 8 пунктов: установку плагина, подключение к MAX, webhook-сервер, health-эндпоинт, подписку, токен, E2E-отправку и настройку reasoning. Exit code: `0` — всё ок, `1` — есть проблемы.
