...
При 3+ кнопках они автоматически нумеруются (`1.`, `2.`, `3.`...) как в тексте, так и на самих кнопках. Текст кнопок дублируется в теле сообщения — чтобы на мобильных устройствах, где MAX может обрезать текст кнопок, содержимое было видно полностью.
